import { RiskDto, RiskImpact, RiskLikelihood, RiskStatus, UpdateRiskDto } from "@/types/projectDetail";
import { deleteRisk, updateRisk } from "@/utils/projectDetailApi";
...
const RiskItem: React.FC<RiskItemProps> = ({ risk, projectId, onRiskChanged }) => {
  ...
  if (isEditing) {
    return (
      <li className="p-4 border border-red-300 rounded-lg bg-red-50 space-y-2 mb-3">
        <h4 className="text-lg font-semibold text-red-800 mb-2">Edit Risk</h4>
        <form onSubmit={handleEditSave} className="space-y-2">
          <div>
            <label htmlFor={`edit-risk-description-${risk.id}`} className="block text-sm font-medium text-gray-700">Description:</label>
            <textarea
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label htmlFor={`edit-risk-impact-${risk.id}`} className="block text-sm font-medium text-gray-700">Impact:</label>
              <select
                id={`edit-risk-impact-${risk.id}`}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm p-2"
                value={editedRisk.impact}
                onChange={(e) => setEditedRisk({ ...editedRisk, impact: Number(e.target.value) as RiskImpact })}
                disabled={isSaving}
              >
                {Object.values(RiskImpact)
                  .filter((value) => typeof value === "number")
                  .map((impact) => (
                    <option key={impact} value={impact}>{RiskImpact[impact]}</option>
                  ))}
              </select>
            </div>
            <div>
              <label htmlFor={`edit-risk-likelihood-${risk.id}`} className="block text-sm font-medium text-gray-700">Likelihood:</label>
              <select
                id={`edit-risk-likelihood-${risk.id}`}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm p-2"
                value={editedRisk.likelihood}
                onChange={(e) => setEditedRisk({ ...editedRisk, likelihood: Number(e.target.value) as RiskLikelihood })}
                disabled={isSaving}
              >
                {Object.values(RiskLikelihood)
                  .filter((value) => typeof value === "number")
                  .map((likelihood) => (
                    <option key={likelihood} value={likelihood}>{RiskLikelihood[likelihood]}</option>
                  ))}
              </select>
            </div>
          </div>
          <div>
            <label htmlFor={`edit-risk-mitigationPlan-${risk.id}`} className="block text-sm font-medium text-gray-700">Mitigation Plan (Optional):</label>
            <textarea
          ...
          <div>
            <label htmlFor={`edit-risk-status-${risk.id}`} className="block text-sm font-medium text-gray-700">Status:</label>
            <select
              id={`edit-risk-status-${risk.id}`}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm p-2"
              value={editedRisk.status}
              onChange={(e) => setEditedRisk({ ...editedRisk, status: Number(e.target.value) as RiskStatus })}
              disabled={isSaving}
            >
              {Object.values(RiskStatus)
                .filter((value) => typeof value === "number")
                .map((status) => (
                  <option key={status} value={status}>{RiskStatus[status]}</option>
                ))}
            </select>
          </div>
    ...
    <li className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-white rounded-lg shadow-sm border border-gray-100 mb-2 last:mb-0">
      <div className="flex-grow">
        <div className="flex flex-wrap items-center gap-2 mb-1 sm:mb-0">
          <strong className="text-gray-800 text-base">{risk.description}</strong>
          <span className="text-sm text-gray-600">
            (Impact: {displayImpact}, Likelihood: {displayLikelihood})
          </span>
          <span className={`font-medium text-sm px-2 py-0.5 rounded-full
            ${risk.status === RiskStatus.Closed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`
          }>
            {displayStatus}
          </span>
        </div>
