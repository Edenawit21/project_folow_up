...
const SprintMetricsCard: React.FC<SprintMetricsCardProps> = ({ report }) => {
  ...
  const metrics = [
    ...
    {
      label: "Story Points",
      value: `${report.completedStoryPoints || 0} / ${
        report.totalStoryPoints || 0
      }`,
    ...
    },
    {
      ...
      icon: <Flame className="text-purple-600" />,
    },
    {
      label: "Tasks Moved From Previous",
  ...
  return (
    <Card title="Sprint Metrics" icon={Gauge}>
      <div className="p-4">
        {" "}
        {/* Added padding to the container of the entire card content */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {metrics.map((metric, index) => (
            <div
              key={index}
              className="flex items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm"
            >
