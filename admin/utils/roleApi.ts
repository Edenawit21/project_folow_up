...
export const fetchRoleById = async (id: string): Promise<RoleData> => {
  const response = await axios.get(`${BASE_URL}/api/roles/${id}`);
...
export const createRole = async (data: Omit<RoleData, "id">) => {
  const response = await axios.post(`${BASE_URL}/api/roles`, data);
  return response.data;
};

// Update an existing role
export const updateRole = async (id: string, data: Omit<RoleData, "id">) => {
  const response = await axios.put(`${BASE_URL}/api/roles/${id}`, data);
  return response.data;
};

// Delete a role
export const deleteRole = async (id: string) => {
  const response = await axios.delete(`${BASE_URL}/api/roles/${id}`);
  return response.data;
};

// Fetch all privileges
export const fetchPrivileges = async () => {
  const response = await axios.get(`${BASE_URL}/api/privileges`);
  return response.data;
};
