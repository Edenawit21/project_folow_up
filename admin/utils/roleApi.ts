import axios from "axios";
import {
  RoleData,
  RoleApiResponse,
  RolePayload,
  RoleUpdatePayload,
} from "@/types/role";

const ROLE_API_URL = process.env.NEXT_PUBLIC_BASE_API_URL;

export const fetchAllRoles = async (token:string): Promise<RoleData[]> => {
  const response = await axios.get<RoleApiResponse>(`${ROLE_API_URL}/api/Role`,{
    headers: {
      Authorization: `Bearer ${token}`, // Add authorization header
    },
  });
  if (!response.data.success) throw new Error("Failed to fetch roles");
  return response.data.data ?? [];
};

export const createRole = async (payload: RolePayload , token:string): Promise<RoleData> => {
  const response = await axios.post(`${ROLE_API_URL}/api/Role`, payload,{
    headers: {
      Authorization: `Bearer ${token}`, // Add authorization header
    },
  });
  if (!response.data.success) throw new Error("Failed to create role");
  return response.data.data;
};

export const fetchRoleById = async (id: string): Promise<RoleData> => {
  const response = await axios.get(`${ROLE_API_URL}/api/Role/${id}`);
  if (!response.data.success) throw new Error("Failed to fetch role");
  return response.data.data;
};
export const updateRole = async (
  id: string,
  data: RolePayload | RoleUpdatePayload,
  token: string
): Promise<RoleData> => {
  const response = await axios.put(`${ROLE_API_URL}/api/Role/${id}`, data, {
    headers: {
      Authorization: `Bearer ${token}`, // Add authorization header
    },
  }
  );
  if (!response.data.success) throw new Error("Failed to update role");
  return response.data.data;
};

export const deleteRole = async (id: string , token:string): Promise<void> => {
  const response = await axios.delete(`${ROLE_API_URL}/api/Role/${id}` ,{
    headers: {
      Authorization: `Bearer ${token}`, // Add authorization header
    },
  });
  if (!response.data.isSuccess) throw new Error("Failed to delete role");
};
